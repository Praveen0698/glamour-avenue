...
export default function Expertise() {
  ...
  return (
    <section
      id="expertise"
      className="w-1/2 py-10 mx-auto rounded-lg bg-white flex flex-col"
    >
      <h1 className="mx-auto text-3xl font-semibold">OUR EXPERTISE</h1>
      <nav className="bg-[#fdfdfd] p-2 border-b border-gray-200 rounded-t-lg my-10">
        <ul className="flex w-full list-none font-medium text-sm m-0 p-0">
          {tabs.map((item) => (
            <motion.li
              key={item.label}
              initial={false}
...
const [NAILS, BROW, HAIR, COSMETOLOGY, MAKEUP] = allIngredients;
const tabs = [NAILS, BROW, HAIR, COSMETOLOGY, MAKEUP];
